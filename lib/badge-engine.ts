...
function isDynamicBadgeCount(badge: BadgeDefinition): boolean {
	const config = badge.requirement?.config ?? {};
	return badge.badgeId === COMPLETIONIST_BADGE_ID || config.dynamic === true;
}

function resolveBadgeCountRequirement(badge: BadgeDefinition, totalEnabledBadges: number): number {
	const config = badge.requirement?.config ?? {};
	const isCompletionist = isDynamicBadgeCount(badge);
...
export class BadgeRuleEngine {
	...
	evaluateAll(badges: BadgeDefinition[], input: WalletBadgeInput): UserBadge[] {
		...
		const staticBadgeCountBadges = badgeCountBadges
			.filter(({ badge }) => !isDynamicBadgeCount(badge))
			.sort((a, b) => a.requiredCount - b.requiredCount);

		const dynamicBadgeCountBadges = badgeCountBadges
			.filter(({ badge }) => isDynamicBadgeCount(badge))
			.sort((a, b) => a.requiredCount - b.requiredCount);

		const baseBadgeCount = earnedBadgeIds.size;

		for (const { badge, requiredCount } of staticBadgeCountBadges) {
			if (baseBadgeCount >= requiredCount && !earnedBadgeIds.has(badge.badgeId)) {
				allEarned.push({
					badgeId: badge.badgeId,
					tokenId: 'system',
					earnedAt: new Date().toISOString(),
		...
		for (const { badge, requiredCount } of dynamicBadgeCountBadges) {
			if (earnedBadgeIds.size >= requiredCount && !earnedBadgeIds.has(badge.badgeId)) {
				allEarned.push({
					badgeId: badge.badgeId,
					tokenId: 'system',
					earnedAt: new Date().toISOString(),
